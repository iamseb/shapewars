...
{	
	...
	public class ProjectLocationToMap : FsmStateAction
	{
		...
		[Tooltip("Location vector in degrees longitude and latitude. Typically stored from Get Location Info action.")]
		...
		[Tooltip("If true all coordinates are normalized (0-1); otherwise coordinates are in pixels.")]
		...
		public override void Reset()
		{
			...
			mapProjection = ProjectLocationToMap.MapProjection.EquidistantCylindrical;
		...
		void DoProjectGPSLocation()
		{
			...
			projectedX.Value = normalized.Value == true ? minX.Value + x : minX.Value + x * Screen.width;
			projectedY.Value = normalized.Value == true ? minY.Value + y : minY.Value + y * Screen.height;
		}
		
		void DoEquidistantCylindrical()
		{
			x = (x + 180f) / (maxLongitude.Value - minLongitude.Value);
			y = (y + 90f) / (maxLatitude.Value - minLatitude.Value);
		}
		...
		void DoMercatorProjection()
		{
			...
			float minY = LatitudeToMercator(minLatitude.Value);
			float maxY = LatitudeToMercator(maxLatitude.Value);
			
			y = (LatitudeToMercator(GPSLocation.Value.y) - minY) / (maxY - minY);
		}
		
		float LatitudeToMercator(float latitudeInDegrees)
		{
			float lat = Mathf.Clamp(latitudeInDegrees, -85, 85);
